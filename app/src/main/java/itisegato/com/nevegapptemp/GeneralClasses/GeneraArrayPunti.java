...
public class GeneraArrayPunti {
    ...
    private Punto[] carica(){
        ...
        for(int i=0; i<punti.length;i++){
            punti[i] = new Punto(names[i], descrizioni[i], immagini[i], altitudini[i], storia[i], turismo[i],fauna[i],flora[i],geologia[i], i, varie[i], link_contatti[i], descrizioniComplete[i]);
        }
        return punti;
    }
